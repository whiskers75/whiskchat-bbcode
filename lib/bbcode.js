...
//
// This version is modified for WhiskTech/whiskchat-server
//
...
exports.parse = function(post, cb) {
    ...
    var tagname_re = /^\/?(?:b|i|u|pre|samp|code|colou?r|noparse|url|link|s|q|c|blockquote|smiley|octocat|me|rcv|size|u?list|li)$/i;
	...
	if(isValidTag(m2)) {
	    ...
	    case "c":
            case "code":
		opentags.push(new taginfo_t(m2l, "</code>"));
		crlf2br = false;
		return "<code>";
	    case "rcv":
		opentags.push(new taginfo_t(m2l, "</span>"));
                return '<span style="text-shadow: 2px 2px 0 rgba(64,64,64,0.4),-2px -2px 0px rgba(64,64,64,0.2); font-size: 1.1em;">';
            case "pre":
		...
		if(!m3 || !number_re.test(m3))
		    m3 = "1";
		opentags.push(new taginfo_t(m2l, "</span>"));
		return "<span style=\"font-size: " + Math.min(Math.max(m3, 0.001), 1) + "em\">";
		...
		opentags.push(new taginfo_t(m2l, ''));
                return "<img src='http://whiskchat.com/static/img/smileys/" + m3 + ".png'>";
            case "noparse":
		noparse = true;
		return "";
		...
		if(m3 && uri_re.test(m3)) {
		    ...
		    urlstart = -1;
                    var tmp = "<a target=\"_blank\" href=\"" + m3.replace('"', '') + "\">";
		    return tmp;
		}
		...
		/*case "img":
		  ...
		  m3 = 500;
                  ...
                  return "<"+m2l+" style='max-height: " + m3 + "px;' src=\"";
		  }
		  
                  return "<"+m2l+" style='max-height: 500px;' src=\"";
		  Images just open up too many flaws.
		*/
            case "octocat":
                opentags.push(new taginfo_t(m2l, " />"));
                
                if (m3) {
                    return "<"+m2l+" style='max-height: 200px;' src=\"http://octodex.github.com/images/" + m3 + ".png\"";
                }
                return "<"+m2l+" style='max-height: 200px;' src=\"http://octodex.github.com/images/codercat.png\"";
            case "q":
            case "blockquote":
